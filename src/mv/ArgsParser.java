...
/**
 * Una clase que hace de intermediario entre la entrada del usuario y la máquina
 * virtual. Configura la máquina conforme a dicha entrada.
 * 
...
public class ArgsParser {
	...
	public ArgsParser() {
		...
		options.addOption(OptionBuilder
				...
				.hasArg().withArgName("asmfile").create("a"));

		options.addOption(OptionBuilder.withLongOpt("in")
				.withDescription("Entrada del programa de la maquina-p.")
				.hasArg().withArgName("infile").create("i"));

		options.addOption(OptionBuilder
				.withLongOpt("mode")
				.withDescription(
						"Modo de funcionamiento (batch | interactive). Por defecto, batch.")
		...
		options.addOption(OptionBuilder
				...
				.hasArg().withArgName("outfile").create("o"));
	}

	/**
	 * Parsea los argumentos de entrada del programa y configura la
	 * entrada/salida de la máquina virtual.
	 * 
	 * @param args
	 *            Argumentos de entrada.
	 * @return Dirección del programa fuente.
	 * @throws ParseException
	 *             Si ha habido algún error al parsear los argumentos.
	 */
	public void parse(String[] args) throws ParseException {
		...
		if (cmd.hasOption("h") || cmd.hasOption("help")) {
			...
			if (!cmd.hasOption("m")
					|| cmd.getOptionValue("m").equalsIgnoreCase("interactive")) {
				if (!cmd.hasOption("a"))
				...
				if (!cmd.hasOption("in")) {
					...
					if (cmd.getOptionValue("in").length() == 0)
						throw new MissingArgumentException(
								this.options.getOption("in"));
				...
				if (!cmd.hasOption("out")) {
					...
					if (cmd.getOptionValue("out").length() == 0)
						throw new MissingArgumentException(
								this.options.getOption("out"));
				...
				throw new UnrecognizedOptionException(
						"Modo incorrecto (parámetro -m|--mode)");
		}
	...
	public void printHelp() {
		...
		formatter
				.printHelp(
						"tp.pr3.mv.Main [-a <asmfile>] [-h] [-i <infile>] [-m <mode>] [-o <outfile>]",
						this.options);
	}
